...
_alerts = {};
_ticker_events = ["BTC_ZEC", "BTC_ETH", "BTC_XMR", "BTC_REP"]
...
def register_alert(bot, update, args):
    ...
    try:
        ...
        update_db_user(_db, "id='%s'" % str(user['id']), "subs='%s'" % json.dumps({'event': event, 'threshold': threshold}))

        bot.sendMessage(update.message.chat_id, text='You have been registed. Try /dota_LM ' + str(user['id']))
...
def main():
    ...
    updater = Updater('223693868:AAFXWdEXwlcAgIas9pP5gUn-NOJiHTw9Ps0')
